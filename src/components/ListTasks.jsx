...
const Section = ({ status, tasks, setTasks, todos, inProgress, closed }) => {
    ...
    return (<div ref={drop} className={`w-64 rounded-md p-2 ${isOver ? 'bg-slate-200' : ""}`}>
        ...
        {tasksToMap.length > 0 && tasksToMap.map((task) => <Task key={task
            .id} task={task} tasks={tasks} setTasks={setTasks} />)}
    </div>);
};
...
const Task = ({ task, tasks, setTasks }) => {
    ...
    return (<div ref={drag} className={`relative p-4 mt-8 ${isDragging ? 'opacity-25' : 'opacity-100'} shadow-md rounded-md cursor-grab`}>
        <button onClick={() => handleRemove(task.id)} className="absolute buttom-1 right-1 text-slate-400">
